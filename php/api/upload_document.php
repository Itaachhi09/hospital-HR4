...
$target_dir_absolute = __DIR__ . '/' . $target_dir_relative; // Build path without resolving yet

// Create the directory if it does not exist
if (!is_dir($target_dir_absolute)) {
    if (!mkdir($target_dir_absolute, 0775, true)) {
        error_log("Failed to create upload directory: " . $target_dir_absolute);
        http_response_code(500);
        echo json_encode(['error' => 'Server configuration error: Unable to create upload directory.']);
        exit;
    }
}
...
// Normalize path to include trailing separator
if (substr($target_dir_absolute, -1) !== DIRECTORY_SEPARATOR) {
